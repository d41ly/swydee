...
| U6 | Canonical account total with provenance | [canonical-total-spec.md](../../../../analysis/builds/2026-07-07-ANLZ-aCanonicalTotal/spec/2026-07-07-spec-aCanonicalTotal-1.md) | shipped |
| U7a | Cross-widget reconciliation #3/#4/#5 | [cross-widget-reconciliation-spec.md](../../../../analysis/builds/2026-07-07-ANLZ-aCrossWidget/spec/2026-07-07-spec-aCrossWidget-1.md) | shipped |
| U7b | Cross-widget reconciliation #6 (monthly-sum vs period KPI) | [cross-widget-reconciliation-spec.md](../../../../analysis/builds/2026-07-07-ANLZ-aCrossWidget/spec/2026-07-07-spec-aCrossWidget-1.md) | shipped (built 2026-07-14 with MF-1..MF-9 folded; `Analyze-SwydoTrend.ps1` new `-PeriodKpiFacts`, `Get-MonthSpan`/`Get-MonthRangeSum`, all findings INFO in v1; additive on TrendAnalyze 24 -> 66) |
| U8 | Extractor period resolution (RELATIVE -> concrete months) | [extractor-period-resolution-spec.md](../../../../extraction/builds/2026-07-13-EXTR-aRelativePeriod/spec/2026-07-13-spec-aRelativePeriod-1.md) | shipped (built + reviewed 2026-07-12 GO-WITH-CHANGES; unattended domain = quarter/-1 only, month/-1 + year/-1 null-triple behind the live-probe gate); supersedes canonical-total-spec.md:5 sequencing (D-period built standalone ahead of U7b) and records the D5 placement deviation (report.dateRangeResolved, not meta) |
| U9 | Headline rank precedence (KPI over doc-earlier table total) | [headline-rank-precedence-spec.md](../../../../analysis/builds/2026-07-13-ANLZ-aHeadlineRank/spec/2026-07-13-spec-aHeadlineRank-1.md) | shipped (built 2nd, after U8, on feat/u8-u9-u10-u7b-build 2026-07-14; a true zero-dim KPI supersedes a doc-earlier table total, `meta.canonicalVersion` 1->2 globally, flip disclosed via `GAP_HEADLINE_SOURCE_CHANGED` + `canonical.supersededWidgetId`; carries the reviewed, disclosed byte-for-byte flip-set waiver — see the note on the byte-for-byte line below) |
| U10 | Data-gap rules: cost-ranking-without-value + branded-baseline-suspect | [data-gap-rules-spec.md](../../../../analysis/builds/2026-07-13-ANLZ-aGappedRanking/spec/2026-07-13-spec-aGappedRanking-1.md) | shipped (built 3rd, after U9, on feat/u8-u9-u10-u7b-build 2026-07-14; `GAP_COST_RANKING_NO_VALUE` major forced-caveat + `ANOM_BRAND_BASELINE` info, both additive-in-facts in Analyze only; rule (a) statement lists explicit cost-per names + a generic pair phrase (never labels raw spend as cost-per) and stays digit-free; `Test-ValueMetricId` target-guarded + `value_per_`-widened; rule (b) brand tokens via a local pure `Get-BrandTokens` on `meta.client` — NOT Manage's `Normalize-ClientName` (D6 clobber-hazard, repo wins); rule (b) may enlarge an existing breakdown's rows/`shown`/`note` via the shipped `$plLabels` force-include on >20-campaign accounts) |

Computation-quality build order (this review): U6 -> U7a -> [U7b deferred].
Post-review build order (2026-07-12 session, declared per the cross-cutting critic): **U8 -> U9 -> U10 -> U7b** — U8 first because U7b's undeferral and U9's canonicalVersion coordination both key on it; U9-before-U10 is arbitrary but binding (both append to `$gaps` at the same post-A:683 seam in `Analyze-SwydoReport.ps1`). Each non-first unit re-derives its `A:` line anchors and re-measures its Analyze green baseline at build time — its "additive on Analyze 276" reads as "additive on the measured post-predecessor count".
...
**Cross-cutting discipline (biggest risk = silent incompleteness/untraceability past a green gate):** every change stays ADDITIVE-IN-FACTS and verifiable by the single-facts closer — carry full provider lists + forced filter caveats (never subtract), keep trend a separate closer pass (never merge two facts files), and default single-report path stays byte-for-byte unchanged (additive `meta` fields only) — except the reviewed, disclosed U9 flip-set waiver (`headline-rank-precedence-spec.md` D1/D3): on a report where a later zero-dim KPI supersedes a doc-earlier table total the flipped cells and their derived facts change, always disclosed in-facts (D4/D5), and `meta.canonicalVersion` bumps 1->2 on every report.
