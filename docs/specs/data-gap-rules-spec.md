...
**Status:** SHIPPED (built 3rd, after U9, on feat/u8-u9-u10-u7b-build 2026-07-14; AMENDMENTS v1 folded in — rule (a) statement rewritten to list only explicit cost-per-outcome names plus a generic digit-free pair phrase and to read "no positive ... recorded"; `Test-ValueMetricId` target-guarded, `value_per_`-widened, `return_on_ad_spend` added; S2 lookahead `(?![\s_-]+new\b)` and S3 space-padded phrase check; brand tokens via local `Get-BrandTokens` on `meta.client`; rule (b) platform name @()-wrap-robust so its `$plLabels` force-include matches on JSON-collapsed single-provider widgets). Reviewed 2026-07-12, verdict GO-WITH-CHANGES. Extends `docs/specs/context-and-canon-spec.md`; same discipline (PS 5.1/.NET Framework, pure-ASCII script source, functions-first + `-DefineOnly` dot-sourcing, hardened scripts never behaviorally modified, default single-report output path byte-for-byte unchanged - every change additive-in-facts, every credential path fail-closed, the model does no arithmetic and every number traces through `skill/scripts/Test-ReportNumbers.ps1`).
...
- The default single-report output path is additive-only: identical facts except (possibly) one new `dataGaps` entry, one new `anomalies` entry, and their fids. `factsVersion` stays 1; the closer ignores unknown finding keys (`confidence` already exists on WIN/LOSS; `requiresDownstreamData` already exists on `ANOM_SHARE_MISMATCH` A:206). **AMENDMENTS correction (discipline must-fix):** the "no existing field changes" claim is false in general — rule (b)'s quoted campaign labels ride the shipped `$plLabels` force-include (A:757-763), so on a >20-campaign account rule (b) can enlarge an existing platform breakdown's `rows`/`shown`/`note` ("showing top N of M"). On the live 8-row QCU widget nothing changes; the mechanism is precedented (every anomaly rule's quoted labels do this). Test 25 is the pin.
