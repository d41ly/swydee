...
**D10. No version bumps: `schemaVersion` stays 2, `factsVersion` stays 1, `canonicalVersion` unchanged BY U8.** The only new version marker is `dateRangeResolved.resolverVersion=1`, scoped to the object it describes.
*(Amended by U9 per headline-rank-precedence-spec.md:10: U8 makes no claim about other units' `canonicalVersion` bumps. U9/D6 bumps `canonicalVersion` 1 -> 2 globally; this D10 asserts only that U8 itself does not touch it.)*
