...
| U7b | Cross-widget reconciliation #6 (monthly-sum vs period KPI) | [cross-widget-reconciliation-spec.md](cross-widget-reconciliation-spec.md) | SPEC, DEFER LIFTED conditionally (re-review vs U8 2026-07-12, GO-WITH-CHANGES) — buildable only after U8 ships as specced AND MF-1..MF-9 (re-review block) are folded into the U7b sections before its build commit; **precondition met by U8** (2026-07-14; `facts.meta.period` now emitted) |
| U8 | Extractor period resolution (RELATIVE -> concrete months) | [extractor-period-resolution-spec.md](extractor-period-resolution-spec.md) | shipped (built + reviewed 2026-07-12 GO-WITH-CHANGES; unattended domain = quarter/-1 only, month/-1 + year/-1 null-triple behind the live-probe gate); supersedes canonical-total-spec.md:5 sequencing (D-period built standalone ahead of U7b) and records the D5 placement deviation (report.dateRangeResolved, not meta) |
| U9 | Headline rank precedence (KPI over doc-earlier table total) | [headline-rank-precedence-spec.md](headline-rank-precedence-spec.md) | shipped (built 2nd, after U8, on feat/u8-u9-u10-u7b-build 2026-07-14; a true zero-dim KPI supersedes a doc-earlier table total, `meta.canonicalVersion` 1->2 globally, flip disclosed via `GAP_HEADLINE_SOURCE_CHANGED` + `canonical.supersededWidgetId`; carries the reviewed, disclosed byte-for-byte flip-set waiver — see the note on the byte-for-byte line below) |
| U10 | Data-gap rules: cost-ranking-without-value + branded-baseline-suspect | [data-gap-rules-spec.md](data-gap-rules-spec.md) | shipped (built 3rd, after U9, on feat/u8-u9-u10-u7b-build 2026-07-14; `GAP_COST_RANKING_NO_VALUE` major forced-caveat + `ANOM_BRAND_BASELINE` info, both additive-in-facts in Analyze only; rule (a) statement lists explicit cost-per names + a generic pair phrase (never labels raw spend as cost-per) and stays digit-free; `Test-ValueMetricId` target-guarded + `value_per_`-widened; rule (b) brand tokens via a local pure `Get-BrandTokens` on `meta.client` — NOT Manage's `Normalize-ClientName` (D6 clobber-hazard, repo wins); rule (b) may enlarge an existing breakdown's rows/`shown`/`note` via the shipped `$plLabels` force-include on >20-campaign accounts) |
