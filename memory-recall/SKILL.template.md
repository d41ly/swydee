...
description: >-
  ...
  ({{FAMILIES}}), or a backlog row. Also use before touching an unfamiliar area, to find the
  records that bind the change. Routes through the shipped CLI, {{QUERY_CLI}}, which needs the
...
```bash
{{QUERY_CLI}} "<the question, in plain English>" --terms "<8-14 coined words>"
```
